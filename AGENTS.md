...
- Arena ammo stations are reload triggers only, never combat-safe zones. Incoming projectile resolution runs before refill/tactical updates each frame, so `refilling` cannot gate, reorder, or suppress valid hits; a player remains fully vulnerable while the fixed 2.5-second refill runs and can be eliminated by the normal Golden Hit rules.
